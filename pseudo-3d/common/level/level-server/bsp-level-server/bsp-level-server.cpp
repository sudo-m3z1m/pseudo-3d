...
{
	if(!node) return 0;
	if (!node->shapes.empty()) return node->shapes[node->shapes.size() - 1]->sector_index;
...
std::vector<BSPShape*> BSPLevelServer::separate_shape_by_line(Line line, BSPShape shape)
{
	...
	for(Wall current_wall : shape_walls)
	{
		...
		{
			new_shapes[points_indeces[0] + points_indeces[1]]->add_new_wall(wall_points[0], wall_points[1], current_wall.normal, current_wall.window_component, current_wall.tid);
			continue;
		}
		if(get_sign(points_k[0]) != get_sign(points_k[1]))
		{
			new_shapes[points_indeces[0]]->add_new_wall(wall_points[0], intersection_point, current_wall.normal, current_wall.window_component, current_wall.tid);
			new_shapes[points_indeces[1]]->add_new_wall(intersection_point, wall_points[1], current_wall.normal, current_wall.window_component, current_wall.tid);
			continue;
		}
		new_shapes[points_indeces[0]]->add_new_wall(wall_points[0], wall_points[1], current_wall.normal, current_wall.window_component, current_wall.tid);
	}
...
void BSPLevelServer::separate_bsp_node(BSPNode* node, std::vector<BSPShape*> node_shapes)
{
	...
	{
		node->shapes.push_back(node_shapes[0]);
		return;
	}
	...
	for(BSPShape*& current_shape : node_shapes)
	{
		for (Wall current_wall : current_shape->walls)
		{
			...
			if(front.empty() || back.empty()) continue;
...
//			node_shapes.clear();
	...
	for(BSPShape*& shape : node_shapes)
	{
