...
int TextureBuffer::load_texture(char* path)
{
...
Color TextureBuffer::get_texture_pixel(int tid, Vector2D<int>& pixel_pos)
{
	...
	SDL_ReadSurfacePixel(buffer[tid], pixel_pos.x, pixel_pos.y, &pixel_color.r, &pixel_color.g, &pixel_color.b, &pixel_color.a);
	return pixel_color;
}
